...
- [x] **0.15** Backend lint debt cleared: `npx eslint --fix` for the ~325 pure-formatting findings, then each real finding fixed at the root cause rather than suppressed — installed missing `@types/bcrypt` (was silently typing every `bcrypt.hash`/`compare` call as `error`, the single biggest source), added a shared `AuthenticatedRequest`/`AuthenticatedUser` type for every `@Req() req: any` handler and guard/decorator reading `req.user`, typed `class-transformer` `@Transform` callbacks and TypeORM `where` builders instead of `any`, and replaced a couple of `any`-typed test mocks/DTO casts with their real DTO types. `test/app.e2e-spec.ts`/`test/setup-e2e.ts` parsing errors fixed by adding `test/**/*` to `tsconfig.json`'s `include` (eslint's `projectService` needs every linted file inside some tsconfig project). `eslint.config.mjs` also now sets `no-unused-vars` with `ignoreRestSiblings: true` so the common `const { password_hash, ...result } = user` pattern doesn't need a rename hack. `npx eslint "{src,apps,libs,test}/**/*.ts"` is 0 problems; `npm run build` and `npm test` (21/21) both still pass. CI's lint step no longer has `continue-on-error` — it gates merges now.
...
- [x] **0.16** Decided: keep `erp_test` — the live Supabase dev/staging project backing the deployed Render backend already has all 6 migrations recorded as applied under that schema, so rewriting the shipped migration SQL to `public` would mean rewriting history a real deployed environment already disagrees with (exactly the risk this step's original note called out). `erp_test` is now documented as the app's permanent schema, not a scratch space.

  What was actually broken — the backend couldn't boot against *any* fresh Postgres (a new Supabase/Render project, CI, a developer's own local Postgres, a from-scratch `migration:run`) because nothing but docker-compose's `init-schema.sql` ever created that schema before migrations ran — is now fixed at the root: `erp-backend/src/database/ensure-schema.ts` creates the schema (`CREATE SCHEMA IF NOT EXISTS`) automatically, called from both `app.module.ts`'s `TypeOrmModule.forRootAsync` (app boot) and `src/data-source.ts` (the CLI path — wraps `AppDataSource.initialize()` since the CLI calls it directly with no factory hook to hang this off). Removed the now-redundant `erp-backend/docker/init-schema.sql` and its docker-compose mount.

  Verified end-to-end against a real, completely fresh, schema-less local Postgres 16 instance (installed directly in this sandbox, docker wasn't available here): dropped to zero schemas beyond `public`, ran `npm run migration:run` — all 6 migrations executed cleanly, `erp_test` created automatically, 30 tables landed. Repeated against a second fresh database booting the full Nest app (`npm run start:prod`) — same result, schema auto-created, app came up and started serving routes. `npm run build`, `npx eslint "{src,apps,libs,test}/**/*.ts"` (0 problems), and `npm test` (21/21) all still pass.
...
- [x] **1.5** BOM approval workflow — `BomStatus` enum (`draft` / `pending_approval` / `active`) replaces the previously-unused free-form `status` column, backed by a DB CHECK constraint (migration `AddBomApprovalWorkflow`, which also normalizes any pre-existing rows — see its comment). Every BOM is now created as a draft (`is_active` also now defaults `false` — it only ever becomes `true` as a side effect of approval, not at creation, which is where the step 1.3 versioning/auto-deactivate logic now runs). `BomController` adds three PATCH transitions: `:id/submit` (draft → pending_approval, no extra role — same access as create/update), `:id/approve` and `:id/reject` (pending_approval → active/back-to-draft, both restricted to `COMPANY_ADMIN`/`SUPERADMIN` via `@Roles` + `RolesGuard` — the maker-checker split). `BomService.update()` now refuses to edit anything past draft (409, "create a new version instead"). No generic workflow engine (that's step 2.3) — just the status field + guard the plan asked for. Frontend (`erp-frontend/src/app/bom/*`) updated to match: create/edit no longer send `status`/`is_active` (removed from `CreateBomDto` — the backend owns those now), the edit page locks its form for non-draft BOMs, and the detail page gained Submit/Approve/Reject buttons wired to the new endpoints. Verified end-to-end against a real fresh Postgres: full API flow (create → edit-while-draft → submit → 403 as a plain USER → 200 as COMPANY_ADMIN → approve) exercised over HTTP with curl, including a second BOM version to confirm the 1.3 auto-deactivate still fires on approve, and a reject sending a third version back to draft. Also fixed a real pre-existing bug found along the way: `app.module.ts`'s and `data-source.ts`'s discrete-host-vars Postgres branch (`DB_HOST`/`DB_PORT`/etc, used when `DATABASE_URL` isn't set) was missing the `schema` option entirely, so any entity repository query — not just BOM's — would fail with `relation "x" does not exist` on that path; every real deployed environment uses `DATABASE_URL` so this never showed up in Render/Supabase, only found because this session's step 0.16 verification happened to use discrete vars for a migration-only check, not a full app-boot-plus-query check. `npm run build`, `npx eslint` (0 problems), and `npm test` (29/29, 8 new BOM workflow tests) all pass.
